...
if enable_lr:
    cbf.use_smoothening = False
    ddplr_dict = run_simulation(linear_sys, cbf, method='ddplr', Rc=1e-8, horizon=100, gamma=0.0)
...
for row_number in range(len(kappavals)):
    ...
    if cbf_type=='A':        
        ...
        axes[row_number, 2].set_xlim([-2.5, 2.5])
...
plt.savefig(f'./linear_sys/cbf_{cbf_type}_filtering_smooth_max.png', bbox_inches="tight", dpi=300)
